...
public class Launcher implements ServiceListener {
	...
	private int run(String args[]) throws Exception {
		try {
            ...
            if (LauncherAgent.instrumentation != null) {
                ...
                framework.getBundleContext().registerService(Instrumentation.class.getName(),
            ...
            framework.getBundleContext().registerService(new String[] {
	...
	private void doTimeoutHandler() {
		...
		Thread wait = new Thread("FrameworkWaiter") {
			...
			public void run() {
				try {
					FrameworkEvent result = framework.waitForStop(parms.timeout);
					if (!active.get()) {
	...
	private void doSecurity() {
		try {
			...
			policy = new SimplePermissionPolicy(this, framework.getBundleContext());
	...
	public void deactivate() throws Exception {
		if (active.getAndSet(false)) {
			framework.stop();
			framework.waitForStop(parms.timeout);
	...
	public void report(PrintStream out) {
		try {
			out.println("------------------------------- REPORT --------------------------");
			out.println();
			row(out, "Framework", framework == null ? "<>" : framework.getClass());
			row(out, "Framework type", "META-INF/services");
	...
	public synchronized void serviceChanged(ServiceEvent event) {
		if (event.getType() == ServiceEvent.REGISTERED) {
			final Object service = framework.getBundleContext().getService(event.getServiceReference());
